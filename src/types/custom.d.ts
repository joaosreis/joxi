import type {
  INordigenBank,
  INordigenBankAccount,
  INordigenBankAccountDetail,
  INordigenBookedTransaction,
} from "./index.d.ts";
...
export type Account =
  & Pick<
    INordigenBankAccount,
    "id" | "created" | "last_accessed" | "iban" | "status" | "owner_name"
  >
  & Pick<
    INordigenBankAccountDetail["account"],
    "currency" | "bban" | "name" | "bic"
  >
  & { page_id: string; transactions: Transaction[]; balanceAmount: number };
export type Bank = Pick<INordigenBank, "id" | "name" | "logo"> & {
