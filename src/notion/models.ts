...
export function txToNotion(
  ...
  const value_date2 = value_date == undefined ? booking_date : booking_date;
...
function getNotionTitle(value?: string): NotionPropertyTitle {
  const annotations: annotationsLookup = {
    bold: false,
    italic: false,
    strikethrough: false,
    underline: false,
...
export type NotionItem<T extends DatabaseType> = T extends
  DatabaseType.Transactions ? NotionTransaction : NotionAccount;
